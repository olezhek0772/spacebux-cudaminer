			  ...
			  void *user_data)
{
	...
	db->buf = newmem;
	db->len = newlen;
	memcpy((void*)((size_t)db->buf + oldlen), ptr, len);
	memcpy((void*)((size_t)db->buf + newlen), &zero, 1);	/* null terminate */
...
struct thread_q *tq_new(void)
{
	...
	tq = (thread_q*)calloc(1, sizeof(*tq));
	if (!tq)
		return NULL;
...
bool tq_push(struct thread_q *tq, void *data)
{
	...
	ent = (tq_ent*)calloc(1, sizeof(*ent));
	if (!ent)
		return false;
